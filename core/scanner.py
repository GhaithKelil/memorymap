...
core/scanner.py
---------------
Scans raw memory bytes for sensitive patterns:
  - Emails, URLs, API keys, JWT tokens, credit card numbers,
    private keys, passwords, AWS/Azure/GCP credentials, and more.
"""
...
@dataclass
class Finding:
    category:    str    # e.g. "API Key", "Email", "JWT Token"
    severity:    str    # CRITICAL / HIGH / MEDIUM / LOW
    match:       str    # the actual matched string (truncated if long)
    offset:      int    # byte offset within the region
    region_base: int    # base address of the region
...
# ── Pattern registry ──────────────────────────────────────────────────────────
#  Each entry: (pattern_name, category, severity, regex_pattern)
...
class SecretScanner:
    """
    Scans a list of MemoryRegion objects for sensitive strings.
    Works on raw bytes — dedupes and scores results.
    """
    ...
    def scan_regions(self, regions) -> List[Finding]:
        """Scan all committed memory regions. Returns a deduplicated list of Findings."""
        all_findings: List[Finding] = []
        seen: set = set()       # deduplicate by (category, match_text)

        for region in regions:
            if not region.data:
                continue
            findings = self._scan_bytes(region.data, region.base_address)
            for f in findings:
